...
namespace SampleClient
{
    public partial class Form_Main : Form
    {
        ...
        {
            InitializeComponent();
            SetPersonsList(Database.Persons.Read());
        }

        private void SetCurrentPerson(int index)
        {
            if (index < 0)
            {
            ...
            textBox_name.Text = _currentPerson.Name as string;
            comboBox_gender.SelectedValue = _currentPerson.Gender;
            dateTimePicker_birthDate.Value = DateTime.Parse(_currentPerson.BirthDate as string);
        }

        private void SetPersonsList(List<Person> persons)
        {
            dataGridView_list.DataSource = persons;
        }
